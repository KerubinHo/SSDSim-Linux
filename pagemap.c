...
/*****************************************************
*∂œ—‘,µ±…Í«Îƒ⁄¥Êø’º‰ ß∞‹ ±£¨ ‰≥ˆ°∞malloc ±‰¡ø√˚ error°±
******************************************************/
void alloc_assert(void *p,char *s)//∂œ—‘
{
	if(p!=NULL) return;
...
/*********************************************************************************
*∂œ—‘
*A£¨∂¡µΩµƒtime_t£¨device£¨lsn£¨size£¨ope∂º<0 ±£¨ ‰≥ˆ°∞trace error:.....°±
*B£¨∂¡µΩµƒtime_t£¨device£¨lsn£¨size£¨ope∂º=0 ±£¨ ‰≥ˆ°∞probable read a blank line°±
**********************************************************************************/
void trace_assert(int64_t time_t,int device,unsigned int lsn,int size,int ope)//∂œ—‘
{
...
/************************************************************************************
*∫Ø ˝µƒπ¶ƒ‹ «∏˘æ›ŒÔ¿Ì“≥∫≈ppn≤È’“∏√ŒÔ¿Ì“≥À˘‘⁄µƒchannel£¨chip£¨die£¨plane£¨block£¨page
*µ√µΩµƒchannel£¨chip£¨die£¨plane£¨block£¨page∑≈‘⁄Ω·ππlocation÷–≤¢◊˜Œ™∑µªÿ÷µ
*************************************************************************************/
struct local *find_location(struct ssd_info *ssd,unsigned int ppn)
{
	...
	/*******************************************************************************
	*page_channel «“ª∏ˆchannel÷–pageµƒ ˝ƒø£¨ ppn/page_channelæÕµ√µΩ¡À‘⁄ƒƒ∏ˆchannel÷–
	*”√Õ¨—˘µƒ∞Ï∑®ø…“‘µ√µΩchip£¨die£¨plane£¨block£¨page
	********************************************************************************/
...
/*****************************************************************************
*’‚∏ˆ∫Ø ˝µƒπ¶ƒ‹ «∏˘æ›≤Œ ˝channel£¨chip£¨die£¨plane£¨block£¨page£¨’“µΩ∏√ŒÔ¿Ì“≥∫≈
*∫Ø ˝µƒ∑µªÿ÷µæÕ «’‚∏ˆŒÔ¿Ì“≥∫≈
******************************************************************************/
unsigned int find_ppn(struct ssd_info * ssd,unsigned int channel,unsigned int chip,unsigned int die,unsigned int plane,unsigned int block,unsigned int page)
{
	...
	int page_channel[100];                  /*’‚∏ˆ ˝◊È¥Ê∑≈µƒ «√ø∏ˆchannelµƒpage ˝ƒø*/
	...
	/*********************************************
	*º∆À„≥ˆplane£¨die£¨chip£¨channel÷–µƒpageµƒ ˝ƒø
	**********************************************/
	page_plane=ssd->parameter->page_block*ssd->parameter->block_plane;
	page_die=page_plane*ssd->parameter->plane_die;
	page_chip=page_die*ssd->parameter->die_chip;
    ...
    /****************************************************************************
	*º∆À„ŒÔ¿Ì“≥∫≈ppn£¨ppn «channel£¨chip£¨die£¨plane£¨block£¨page÷–page∏ˆ ˝µƒ◊‹∫Õ
	*****************************************************************************/
	i=0;
	while(i<channel)
	{
...
struct ssd_info *pre_process_page(struct ssd_info *ssd)
{
	...
	if(ssd->tracefile == NULL )      /*¥Úø™traceŒƒº˛¥”÷–∂¡»°«Î«Û*/
	{
	...
	while(fgets(buffer_request,200,ssd->tracefile))
	{
		...
		fl++;
		trace_assert(time,device,lsn,size,ope);                         /*∂œ—‘£¨µ±∂¡µΩµƒtime£¨device£¨lsn£¨size£¨ope≤ª∫œ∑® ±æÕª·¥¶¿Ì*/

		add_size=0;                                                     /*add_size «’‚∏ˆ«Î«Û“—æ≠‘§¥¶¿Ìµƒ¥Û–°*/

		if(ope==1)                                                      /*’‚¿Ô÷ª «∂¡«Î«Ûµƒ‘§¥¶¿Ì£¨–Ë“™Ã·«∞Ω´œ‡”¶Œª÷√µƒ–≈œ¢Ω¯––œ‡”¶–ﬁ∏ƒ*/
		{
			while(add_size<size)
			{				
				lsn=lsn%largest_lsn;                                    /*∑¿÷πªÒµ√µƒlsn±»◊Ó¥Ûµƒlsnªπ¥Û*/		
				sub_size=ssd->parameter->subpage_page-(lsn%ssd->parameter->subpage_page);		
				if(add_size+sub_size>=size)                             /*÷ª”–µ±“ª∏ˆ«Î«Ûµƒ¥Û–°–°”⁄“ª∏ˆpageµƒ¥Û–° ±ªÚ’ﬂ «¥¶¿Ì“ª∏ˆ«Î«Ûµƒ◊Ó∫Û“ª∏ˆpage ±ª·≥ˆœ÷’‚÷÷«Èøˆ*/
				{		
					sub_size=size-add_size;		
					add_size+=sub_size;		
				}

				if((sub_size>ssd->parameter->subpage_page)||(add_size>size))/*µ±‘§¥¶¿Ì“ª∏ˆ◊”¥Û–° ±£¨’‚∏ˆ¥Û–°¥Û”⁄“ª∏ˆpageªÚ «“—æ≠¥¶¿Ìµƒ¥Û–°¥Û”⁄sizeæÕ±®¥Ì*/		
				{		
                ...
                /*******************************************************************************************************
				*¿˚”√¬ﬂº≠…»«¯∫≈lsnº∆À„≥ˆ¬ﬂº≠“≥∫≈lpn
				*≈–∂œ’‚∏ˆdram÷–”≥…‰±Ìmap÷–‘⁄lpnŒª÷√µƒ◊¥Ã¨
				*A£¨’‚∏ˆ◊¥Ã¨==0£¨±Ì æ“‘«∞√ª”––¥π˝£¨œ÷‘⁄–Ë“™÷±Ω”Ω´ub_size¥Û–°µƒ◊”“≥–¥Ω¯»•–¥Ω¯»•
				*B£¨’‚∏ˆ◊¥Ã¨>0£¨±Ì æ£¨“‘«∞”––¥π˝£¨’‚–Ë“™Ω¯“ª≤Ω±»Ωœ◊¥Ã¨£¨“ÚŒ™–¬–¥µƒ◊¥Ã¨ø…“‘”Î“‘«∞µƒ◊¥Ã¨”–÷ÿµ˛µƒ…»«¯µƒµÿ∑Ω
				********************************************************************************************************/
				lpn=lsn/ssd->parameter->subpage_page;
				if(ssd->dram->map->map_entry[lpn].state==0)                 /*◊¥Ã¨Œ™0µƒ«Èøˆ*/
				{
					/**************************************************************
					*ªÒµ√¿˚”√get_ppn_for_pre_process∫Ø ˝ªÒµ√ppn£¨‘Ÿµ√µΩlocation
					*–ﬁ∏ƒssdµƒœ‡πÿ≤Œ ˝£¨dramµƒ”≥…‰±Ìmap£¨“‘º∞locationœ¬µƒpageµƒ◊¥Ã¨
					***************************************************************/
				...
				else if(ssd->dram->map->map_entry[lpn].state>0)           /*◊¥Ã¨≤ªŒ™0µƒ«Èøˆ*/
				{
					map_entry_new=set_entry_state(ssd,lsn,sub_size);      /*µ√µΩ–¬µƒ◊¥Ã¨£¨≤¢”Î‘≠¿¥µƒ◊¥Ã¨œ‡ªÚµƒµΩ“ª∏ˆ◊¥Ã¨*/
				...
				lsn=lsn+sub_size;                                         /*œ¬∏ˆ◊”«Î«Ûµƒ∆ ºŒª÷√*/
				add_size+=sub_size;                                       /*“—æ≠¥¶¿Ì¡Àµƒadd_size¥Û–°±‰ªØ*/
			}//while(add_size<size)
		}//if(ope==1) 
	}	
...
unsigned int get_ppn_for_pre_process(struct ssd_info *ssd,unsigned int lsn)     
{
	...
	if (ssd->parameter->allocation_scheme==0)                           /*∂ØÃ¨∑Ω Ωœ¬ªÒ»°ppn*/
	{
		if (ssd->parameter->dynamic_allocation==0)                      /*±Ì æ»´∂ØÃ¨∑Ω Ωœ¬£¨“≤æÕ «channel£¨chip£¨die£¨plane£¨blockµ»∂º «∂ØÃ¨∑÷≈‰*/
		{
			channel=ssd->token;
		...
		else if (ssd->parameter->dynamic_allocation==1)                 /*±Ì æ∞Î∂ØÃ¨∑Ω Ω£¨channelæ≤Ã¨∏¯≥ˆ£¨package£¨die£¨plane∂ØÃ¨∑÷≈‰*/                 
		{
	...
	else if (ssd->parameter->allocation_scheme==1)                       /*±Ì ææ≤Ã¨∑÷≈‰£¨Õ¨ ±“≤”–0,1,2,3,4,5’‚6÷–≤ªÕ¨æ≤Ã¨∑÷≈‰∑Ω Ω*/
	{
	...
	/******************************************************************************
	*∏˘æ›…œ ˆ∑÷≈‰∑Ω∑®’“µΩchannel£¨chip£¨die£¨plane∫Û£¨‘Ÿ‘⁄’‚∏ˆ¿Ô√Ê’“µΩactive_block
	*Ω”◊≈ªÒµ√ppn
	******************************************************************************/
...
/***************************************************************************************************
*∫Ø ˝π¶ƒ‹ «‘⁄À˘∏¯µƒchannel£¨chip£¨die£¨plane¿Ô√Ê’“µΩ“ª∏ˆactive_block»ª∫Û‘Ÿ‘⁄’‚∏ˆblock¿Ô√Ê’“µΩ“ª∏ˆ“≥£¨
*‘Ÿ¿˚”√find_ppn’“µΩppn°£
****************************************************************************************************/
struct ssd_info *get_ppn(struct ssd_info *ssd,unsigned int channel,unsigned int chip,unsigned int die,unsigned int plane,struct sub_request *sub)
{
	...
	/*************************************************************************************
	*¿˚”√∫Ø ˝find_active_block‘⁄channel£¨chip£¨die£¨plane’“µΩªÓ‘æblock
	*≤¢«“–ﬁ∏ƒ’‚∏ˆchannel£¨chip£¨die£¨plane£¨active_blockœ¬µƒlast_write_page∫Õfree_page_num
	**************************************************************************************/
	if(find_active_block(ssd,channel,chip,die,plane)==FAILURE)                      
	{
	...
	else                                                                            /*’‚∏ˆ¬ﬂº≠“≥Ω¯––¡À∏¸–¬£¨–Ë“™Ω´‘≠¿¥µƒ“≥÷√Œ™ ß–ß*/
	{
		...
		ssd->channel_head[location->channel].chip_head[location->chip].die_head[location->die].plane_head[location->plane].blk_head[location->block].page_head[location->page].valid_state=0;             /*±Ì æƒ≥“ª“≥ ß–ß£¨Õ¨ ±±Íº«valid∫Õfree◊¥Ã¨∂ºŒ™0*/
		ssd->channel_head[location->channel].chip_head[location->chip].die_head[location->die].plane_head[location->plane].blk_head[location->block].page_head[location->page].free_state=0;              /*±Ì æƒ≥“ª“≥ ß–ß£¨Õ¨ ±±Íº«valid∫Õfree◊¥Ã¨∂ºŒ™0*/
		ssd->channel_head[location->channel].chip_head[location->chip].die_head[location->die].plane_head[location->plane].blk_head[location->block].page_head[location->page].lpn=0;
		ssd->channel_head[location->channel].chip_head[location->chip].die_head[location->die].plane_head[location->plane].blk_head[location->block].invalid_page_num++;
		
		/*******************************************************************************************
		*∏√block÷–»´ «invalidµƒ“≥£¨ø…“‘÷±Ω”…æ≥˝£¨æÕ‘⁄¥¥Ω®“ª∏ˆø…≤¡≥˝µƒΩ⁄µ„£¨π“‘⁄locationœ¬µƒplaneœ¬√Ê
		********************************************************************************************/
	...
	sub->ppn=ssd->dram->map->map_entry[lpn].pn;                                      /*–ﬁ∏ƒsub◊”«Î«Ûµƒppn£¨locationµ»±‰¡ø*/
	...
	ssd->program_count++;                                                           /*–ﬁ∏ƒssdµƒprogram_count,free_pageµ»±‰¡ø*/
	...
	if (ssd->parameter->active_write==0)                                            /*»Áπ˚√ª”–÷˜∂Ø≤ﬂ¬‘£¨÷ª≤…”√gc_hard_threshold£¨≤¢«“Œﬁ∑®÷–∂œGCπ˝≥Ã*/
	{                                                                               /*»Áπ˚plane÷–µƒfree_pageµƒ ˝ƒø…Ÿ”⁄gc_hard_thresholdÀ˘…Ë∂®µƒ„–÷µæÕ≤˙…˙gc≤Ÿ◊˜*/
...
/*****************************************************************************************
*’‚∏ˆ∫Ø ˝π¶ƒ‹ «Œ™gc≤Ÿ◊˜—∞’“–¬µƒppn£¨“ÚŒ™‘⁄gc≤Ÿ◊˜÷––Ë“™’“µΩ–¬µƒŒÔ¿ÌøÈ¥Ê∑≈‘≠¿¥ŒÔ¿ÌøÈ…œµƒ ˝æ›
*‘⁄gc÷–—∞’“–¬ŒÔ¿ÌøÈµƒ∫Ø ˝£¨≤ªª·“˝∆—≠ª∑µƒgc≤Ÿ◊˜
******************************************************************************************/
...
/*********************************************************************************************************************
* ÷Ï÷æ√˜ ”⁄2011ƒÍ7‘¬28»’–ﬁ∏ƒ   
*∫Ø ˝µƒπ¶ƒ‹æÕ «erase_operation≤¡≥˝≤Ÿ◊˜£¨∞—channel£¨chip£¨die£¨planeœ¬µƒblock≤¡≥˝µÙ
*“≤æÕ «≥ı ºªØ’‚∏ˆblockµƒœ‡πÿ≤Œ ˝£¨eg£∫free_page_num=page_block£¨invalid_page_num=0£¨last_write_page=-1£¨erase_count++
*ªπ”–’‚∏ˆblockœ¬√Êµƒ√ø∏ˆpageµƒœ‡πÿ≤Œ ˝“≤“™–ﬁ∏ƒ°£
*********************************************************************************************************************/
...
/**************************************************************************************
*’‚∏ˆ∫Ø ˝µƒπ¶ƒ‹ «¥¶¿ÌINTERLEAVE_TWO_PLANE£¨INTERLEAVE£¨TWO_PLANE£¨NORMALœ¬µƒ≤¡≥˝µƒ≤Ÿ◊˜°£
***************************************************************************************/
Status erase_planes(struct ssd_info * ssd, unsigned int channel, unsigned int chip, unsigned int die1, unsigned int plane1,unsigned int command)
{
	...
	if((ssd->channel_head[channel].chip_head[chip].die_head[die1].plane_head[plane1].erase_node==NULL)||               
		((command!=INTERLEAVE_TWO_PLANE)&&(command!=INTERLEAVE)&&(command!=TWO_PLANE)&&(command!=NORMAL)))     /*»Áπ˚√ª”–≤¡≥˝≤Ÿ◊˜£¨ªÚ’ﬂcommand≤ª∂‘£¨∑µªÿ¥ÌŒÛ*/           
	{
		return ERROR;
	}

	/************************************************************************************************************
	*¥¶¿Ì≤¡≥˝≤Ÿ◊˜ ±£¨ ◊œ»“™¥´ÀÕ≤¡≥˝√¸¡Ó£¨’‚ «channel£¨chip¥¶”⁄¥´ÀÕ√¸¡Óµƒ◊¥Ã¨£¨º¥CHANNEL_TRANSFER£¨CHIP_ERASE_BUSY
	*œ¬“ª◊¥Ã¨ «CHANNEL_IDLE£¨CHIP_IDLE°£
	*************************************************************************************************************/
	...
	if(command==INTERLEAVE_TWO_PLANE)                                       /*∏ﬂº∂√¸¡ÓINTERLEAVE_TWO_PLANEµƒ¥¶¿Ì*/
	{
		for(die=0;die<ssd->parameter->die_chip;die++)
		{
			...
			for (plane=0;plane<ssd->parameter->plane_die;plane++)
			{
				...
				if(direct_erase_node!=NULL)
				{
					...
					erase_operation(ssd,channel,chip,die,plane,block);     /*’Ê µµƒ≤¡≥˝≤Ÿ◊˜µƒ¥¶¿Ì*/
		...
		ssd->interleave_mplane_erase_count++;                             /*∑¢ÀÕ¡À“ª∏ˆinterleave two plane erase√¸¡Ó,≤¢º∆À„’‚∏ˆ¥¶¿Ìµƒ ±º‰£¨“‘º∞œ¬“ª∏ˆ◊¥Ã¨µƒ ±º‰*/
	...
	else if(command==INTERLEAVE)                                          /*∏ﬂº∂√¸¡ÓINTERLEAVEµƒ¥¶¿Ì*/
	{
		for(die=0;die<ssd->parameter->die_chip;die++)
		{
			for (plane=0;plane<ssd->parameter->plane_die;plane++)
	...
	else if(command==TWO_PLANE)                                          /*∏ﬂº∂√¸¡ÓTWO_PLANEµƒ¥¶¿Ì*/
	{
	...
	else if(command==NORMAL)                                             /*∆’Õ®√¸¡ÓNORMALµƒ¥¶¿Ì*/
	{
...
/*******************************************************************************************************************
*GC≤Ÿ◊˜”…ƒ≥∏ˆplaneµƒfreeøÈ…Ÿ”⁄„–÷µΩ¯––¥•∑¢£¨µ±ƒ≥∏ˆplane±ª¥•∑¢ ±£¨GC≤Ÿ◊˜’ºæ›’‚∏ˆplaneÀ˘‘⁄µƒdie£¨“ÚŒ™die «“ª∏ˆ∂¿¡¢µ•‘™°£
*∂‘“ª∏ˆdieµƒGC≤Ÿ◊˜£¨æ°¡ø◊ˆµΩÀƒ∏ˆplaneÕ¨ ±erase£¨¿˚”√interleave erase≤Ÿ◊˜°£GC≤Ÿ◊˜”¶∏√◊ˆµΩø…“‘ÀÊ ±Õ£÷π£®“∆∂Ø ˝æ›∫Õ≤¡≥˝
* ±≤ª––£¨µ´ «º‰œ∂ ±º‰ø…“‘Õ£÷πGC≤Ÿ◊˜£©£¨“‘∑˛ŒÒ–¬µΩ¥Ôµƒ«Î«Û£¨µ±«Î«Û∑˛ŒÒÕÍ∫Û£¨¿˚”√«Î«Ûº‰œ∂ ±º‰£¨ºÃ–¯GC≤Ÿ◊˜°£ø…“‘…Ë÷√¡Ω∏ˆ
*GC„–÷µ£¨“ª∏ˆ»Ì„–÷µ£¨“ª∏ˆ”≤„–÷µ°£»Ì„–÷µ±Ì æµΩ¥Ô∏√„–÷µ∫Û£¨ø…“‘ø™ º÷˜∂ØµƒGC≤Ÿ◊˜£¨¿˚”√º‰–™ ±º‰£¨GCø…“‘±ª–¬µΩµƒ«Î«Û÷–∂œ£ª
*µ±µΩ¥Ô”≤„–÷µ∫Û£¨«ø÷∆–‘÷¥––GC≤Ÿ◊˜£¨«“¥ÀGC≤Ÿ◊˜≤ªƒ‹±ª÷–∂œ£¨÷±µΩªÿµΩ”≤„–÷µ“‘…œ°£
*‘⁄’‚∏ˆ∫Ø ˝¿Ô√Ê£¨’“≥ˆ’‚∏ˆdieÀ˘”–µƒplane÷–£¨”–√ª”–ø…“‘÷±Ω”…æ≥˝µƒblock£¨“™ «”–µƒª∞£¨¿˚”√interleave two plane√¸¡Ó£¨…æ≥˝
*’‚–©block£¨∑Ò‘Ú”–∂‡…Ÿplane”–’‚÷÷÷±Ω”…æ≥˝µƒblockæÕÕ¨ ±…æ≥˝£¨≤ª––µƒª∞£¨◊Ó≤ÓæÕ «µ•∂¿’‚∏ˆplaneΩ¯––…æ≥˝£¨¡¨’‚“≤≤ª¬˙◊„µƒª∞£¨
*÷±Ω”Ã¯≥ˆ£¨µΩgc_parallelism∫Ø ˝Ω¯––Ω¯“ª≤ΩGC≤Ÿ◊˜°£∏√∫Ø ˝—∞’“»´≤øŒ™invalidµƒøÈ£¨÷±Ω”…æ≥˝£¨’“µΩø…÷±Ω”…æ≥˝µƒ∑µªÿ1£¨√ª”–’“
*µΩ∑µªÿ-1°£
*********************************************************************************************************************/
int gc_direct_erase(struct ssd_info *ssd,unsigned int channel,unsigned int chip,unsigned int die,unsigned int plane)     
{
	unsigned int lv_die=0,lv_plane=0;                                                           /*Œ™±‹√‚÷ÿ√˚∂¯ π”√µƒæ÷≤ø±‰¡ø Local variables*/
	...
	/********************************************************************************************************
	*µ±ƒ‹¥¶¿ÌTWOPLANE∏ﬂº∂√¸¡Ó ±£¨æÕ‘⁄œ‡”¶µƒchannel£¨chip£¨die÷–¡Ω∏ˆ≤ªÕ¨µƒplane’“µΩø…“‘÷¥––TWOPLANE≤Ÿ◊˜µƒblock
	*≤¢÷√muilt_plane_flagŒ™TRUE
	*********************************************************************************************************/
	...
	/***************************************************************************************
	*µ±ƒ‹¥¶¿ÌINTERLEAVE∏ﬂº∂√¸¡Ó ±£¨æÕ‘⁄œ‡”¶µƒchannel£¨chip’“µΩø…“‘÷¥––INTERLEAVEµƒ¡Ω∏ˆblock
	*≤¢÷√interleaver_flagŒ™TRUE
	****************************************************************************************/
	...
	/************************************************************************************************************************
	*A£¨»Áπ˚º»ø…“‘÷¥––twoplaneµƒ¡Ω∏ˆblock”÷”–ø…“‘÷¥––interleaverµƒ¡Ω∏ˆblock£¨ƒ«√¥æÕ÷¥––INTERLEAVE_TWO_PLANEµƒ∏ﬂº∂√¸¡Ó≤¡≥˝≤Ÿ◊˜
	*B£¨»Áπ˚÷ª”–ƒ‹÷¥––interleaverµƒ¡Ω∏ˆblock£¨ƒ«√¥æÕ÷¥––INTERLEAVE∏ﬂº∂√¸¡Óµƒ≤¡≥˝≤Ÿ◊˜
	*C£¨»Áπ˚÷ª”–ƒ‹÷¥––TWO_PLANEµƒ¡Ω∏ˆblock£¨ƒ«√¥æÕ÷¥––TWO_PLANE∏ﬂº∂√¸¡Óµƒ≤¡≥˝≤Ÿ◊˜
	*D£¨√ª”–…œ ˆ’‚–©«Èøˆ£¨ƒ«√¥æÕ÷ªƒ‹πª÷¥––∆’Õ®µƒ≤¡≥˝≤Ÿ◊˜¡À
	*************************************************************************************************************************/
	...
	if ((normal_erase_flag==TRUE))                              /*≤ª «√ø∏ˆplane∂º”–ø…“‘÷±Ω”…æ≥˝µƒblock£¨÷ª∂‘µ±«∞planeΩ¯––∆’Õ®µƒerase≤Ÿ◊˜£¨ªÚ’ﬂ÷ªƒ‹÷¥––∆’Õ®√¸¡Ó*/
	{
		...
		{
			return FAILURE;                                     /*ƒø±Íµƒplane√ª”–ø…“‘÷±Ω”…æ≥˝µƒblock£¨–Ë“™—∞’“ƒø±Í≤¡≥˝øÈ∫Û‘⁄ µ ©≤¡≥˝≤Ÿ◊˜*/
		}
	}
	return SUCCESS;
}
...
Status move_page(struct ssd_info * ssd, struct local *location, unsigned int * transfer_size)
{
	...
	old_ppn=find_ppn(ssd,location->channel,location->chip,location->die,location->plane,location->block,location->page);      /*º«¬º’‚∏ˆ”––ß“∆∂Ø“≥µƒppn£¨∂‘±»mapªÚ’ﬂ∂ÓÕ‚”≥…‰πÿœµ÷–µƒppn£¨Ω¯––…æ≥˝∫ÕÃÌº”≤Ÿ◊˜*/
	ppn=get_ppn_for_gc(ssd,location->channel,location->chip,location->die,location->plane);                /*’“≥ˆ¿¥µƒppn“ª∂® «‘⁄∑¢…˙gc≤Ÿ◊˜µƒplane÷–,≤≈ƒ‹ π”√copyback≤Ÿ◊˜£¨Œ™gc≤Ÿ◊˜ªÒ»°ppn*/

	new_location=find_location(ssd,ppn);                                                                   /*∏˘æ›–¬ªÒµ√µƒppnªÒ»°new_location*/

	if ((ssd->parameter->advanced_commands&AD_COPYBACK)==AD_COPYBACK)
	{
		if (ssd->parameter->greed_CB_ad==1)                                                                /*Ã∞¿∑µÿ π”√∏ﬂº∂√¸¡Ó*/
		{
			ssd->copy_back_count++;
			ssd->gc_copy_back++;
			while (old_ppn%2!=ppn%2)
			{
				...
				ppn=get_ppn_for_gc(ssd,location->channel,location->chip,location->die,location->plane);    /*’“≥ˆ¿¥µƒppn“ª∂® «‘⁄∑¢…˙gc≤Ÿ◊˜µƒplane÷–£¨≤¢«“¬˙◊„∆Ê≈ºµÿ÷∑œﬁ÷∆,≤≈ƒ‹ π”√copyback≤Ÿ◊˜*/
				ssd->program_count--;
				ssd->write_flash_count--;
	...
	if (old_ppn==ssd->dram->map->map_entry[lpn].pn)                                                     /*–ﬁ∏ƒ”≥…‰±Ì*/
	{
		ssd->dram->map->map_entry[lpn].pn=ppn;
	}
...
/*******************************************************************************************************************************************
*ƒø±Íµƒplane√ª”–ø…“‘÷±Ω”…æ≥˝µƒblock£¨–Ë“™—∞’“ƒø±Í≤¡≥˝øÈ∫Û‘⁄ µ ©≤¡≥˝≤Ÿ◊˜£¨”√‘⁄≤ªƒ‹÷–∂œµƒgc≤Ÿ◊˜÷–£¨≥…π¶…æ≥˝“ª∏ˆøÈ£¨∑µªÿ1£¨√ª”–…æ≥˝“ª∏ˆøÈ∑µªÿ-1
*‘⁄’‚∏ˆ∫Ø ˝÷–£¨≤ª”√øº¬«ƒø±Íchannel,die «∑Ò «ø’œ–µƒ,≤¡≥˝invalid_page_num◊Ó∂‡µƒblock
********************************************************************************************************************************************/
int uninterrupt_gc(struct ssd_info *ssd,unsigned int channel,unsigned int chip,unsigned int die,unsigned int plane)       
{
	...
	unsigned int block,active_block,transfer_size,free_page,page_move_count=0;                           /*º«¬º ß–ß“≥◊Ó∂‡µƒøÈ∫≈*/
	struct local *  location=NULL;
	unsigned int total_invalid_page_num=0;

	if(find_active_block(ssd,channel,chip,die,plane)!=SUCCESS)                                           /*ªÒ»°ªÓ‘æøÈ*/
	{
	...
	transfer_size=0;
	block=-1;
	for(i=0;i<ssd->parameter->block_plane;i++)                                                           /*≤È’“◊Ó∂‡invalid_pageµƒøÈ∫≈£¨“‘º∞◊Ó¥Ûµƒinvalid_page_num*/
	{	
	...
	free_page=0;
	for(i=0;i<ssd->parameter->page_block;i++)		                                                     /*÷∏ˆºÏ≤È√ø∏ˆpage£¨»Áπ˚”–”––ß ˝æ›µƒpage–Ë“™“∆∂ØµΩ∆‰À˚µÿ∑Ω¥Ê¥¢*/		
	{		
		...
		if(ssd->channel_head[channel].chip_head[chip].die_head[die].plane_head[plane].blk_head[block].page_head[i].valid_state>0) /*∏√“≥ «”––ß“≥£¨–Ë“™copyback≤Ÿ◊˜*/		
		{	
			location=(struct local * )malloc(sizeof(struct local ));
			alloc_assert(location,"location");
			memset(location,0, sizeof(struct local));
			...
			location->page=i;
			move_page( ssd, location, &transfer_size);                                                   /*’Ê µµƒmove_page≤Ÿ◊˜*/
			page_move_count++;
	...
	erase_operation(ssd,channel ,chip , die,plane ,block);	                                              /*÷¥––ÕÍmove_page≤Ÿ◊˜∫Û£¨æÕ¡¢º¥÷¥––blockµƒ≤¡≥˝≤Ÿ◊˜*/
	...
	/***************************************************************
	*‘⁄ø…÷¥––COPYBACK∏ﬂº∂√¸¡Ó”Î≤ªø…÷¥––COPYBACK∏ﬂº∂√¸¡Ó’‚¡Ω÷÷«Èøˆœ¬£¨
	*channelœ¬∏ˆ◊¥Ã¨ ±º‰µƒº∆À„£¨“‘º∞chipœ¬∏ˆ◊¥Ã¨ ±º‰µƒº∆À„
	***************************************************************/
	if ((ssd->parameter->advanced_commands&AD_COPYBACK)==AD_COPYBACK)
	{
...
/*******************************************************************************************************************************************
*ƒø±Íµƒplane√ª”–ø…“‘÷±Ω”…æ≥˝µƒblock£¨–Ë“™—∞’“ƒø±Í≤¡≥˝øÈ∫Û‘⁄ µ ©≤¡≥˝≤Ÿ◊˜£¨”√‘⁄ø…“‘÷–∂œµƒgc≤Ÿ◊˜÷–£¨≥…π¶…æ≥˝“ª∏ˆøÈ£¨∑µªÿ1£¨√ª”–…æ≥˝“ª∏ˆøÈ∑µªÿ-1
*‘⁄’‚∏ˆ∫Ø ˝÷–£¨≤ª”√øº¬«ƒø±Íchannel,die «∑Ò «ø’œ–µƒ
********************************************************************************************************************************************/
int interrupt_gc(struct ssd_info *ssd,unsigned int channel,unsigned int chip,unsigned int die,unsigned int plane,struct gc_operation *gc_node)        
{
	...
	if (ssd->channel_head[channel].chip_head[chip].die_head[die].plane_head[plane].blk_head[gc_node->block].invalid_page_num!=ssd->parameter->page_block)     /*ªπ–Ë“™÷¥––copyback≤Ÿ◊˜*/
	{
	...
	else
	{
		...
		return 1;                                                                      /*∏√gc≤Ÿ◊˜ÕÍ≥…£¨∑µªÿ1£¨ø…“‘Ω´channel…œµƒgc«Î«ÛΩ⁄µ„…æ≥˝*/
	}
...
/*************************************************************
*∫Ø ˝µƒπ¶ƒ‹ «µ±¥¶¿ÌÕÍ“ª∏ˆgc≤Ÿ◊˜ ±£¨–Ë“™∞—gc¡¥…œµƒgc_node…æ≥˝µÙ
**************************************************************/
...
Status gc_for_channel(struct ssd_info *ssd, unsigned int channel)
{
	...
	/*******************************************************************************************
	*≤È’“√ø“ª∏ˆgc_node£¨ªÒ»°gc_nodeÀ˘‘⁄µƒchipµƒµ±«∞◊¥Ã¨£¨œ¬∏ˆ◊¥Ã¨£¨œ¬∏ˆ◊¥Ã¨µƒ‘§º∆ ±º‰
	*»Áπ˚µ±«∞◊¥Ã¨ «ø’œ–£¨ªÚ «œ¬∏ˆ◊¥Ã¨ «ø’œ–∂¯œ¬∏ˆ◊¥Ã¨µƒ‘§º∆ ±º‰–°”⁄µ±«∞ ±º‰£¨≤¢«“ «≤ªø…÷–∂œµƒgc
	*ƒ«√¥æÕflag_priority¡ÓŒ™1£¨∑Ò‘ÚŒ™0
	********************************************************************************************/
	...
	while (gc_node!=NULL)
	{
		...
		{
			if (gc_node->priority==GC_UNINTERRUPT)                                     /*’‚∏ˆgc«Î«Û «≤ªø…÷–∂œµƒ£¨”≈œ»∑˛ŒÒ’‚∏ˆgc≤Ÿ◊˜*/
			{
				flag_priority=1;
				break;
			}
	...
	if (flag_priority!=1)                                                              /*√ª”–’“µΩ≤ªø…÷–∂œµƒgc«Î«Û£¨ ◊œ»÷¥––∂” ◊µƒgc«Î«Û*/
	{
		gc_node=ssd->channel_head[channel].gc_command;
		while (gc_node!=NULL)
		{
			...
			next_state_predict_time=ssd->channel_head[channel].chip_head[gc_node->chip].next_state_predict_time;
			 /**********************************************
			 *–Ë“™gc≤Ÿ◊˜µƒƒø±Íchip «ø’œ–µƒ£¨≤≈ø…“‘Ω¯––gc≤Ÿ◊˜
			 ***********************************************/
			if((current_state==CHIP_IDLE)||((next_state==CHIP_IDLE)&&(next_state_predict_time<=ssd->current_time)))   
			{
				break;
	...
	if (gc_node->priority==GC_UNINTERRUPT)
	{
		...
		{
			flag_gc=uninterrupt_gc(ssd,channel,chip,die,plane);                         /*µ±“ª∏ˆÕÍ’˚µƒgc≤Ÿ◊˜ÕÍ≥… ±£®“—æ≠≤¡≥˝“ª∏ˆøÈ£¨ªÿ ’¡À“ª∂® ˝¡øµƒflashø’º‰£©£¨∑µªÿ1£¨Ω´channel…œœ‡”¶µƒgc≤Ÿ◊˜«Î«ÛΩ⁄µ„…æ≥˝*/
			if (flag_gc==1)
			{
	...
	/*******************************************************************************
	*ø…÷–∂œµƒgc«Î«Û£¨–Ë“™ ◊œ»»∑»œ∏√channel…œ√ª”–◊”«Î«Û‘⁄’‚∏ˆ ±øÃ–Ë“™ π”√’‚∏ˆchannel£¨
	*√ª”–µƒª∞£¨‘⁄÷¥––gc≤Ÿ◊˜£¨”–µƒª∞£¨≤ª÷¥––gc≤Ÿ◊˜
	********************************************************************************/
	else        
	{
		flag_invoke_gc=decide_gc_invoke(ssd,channel);                                  /*≈–∂œ «∑Ò”–◊”«Î«Û–Ë“™channel£¨»Áπ˚”–◊”«Î«Û–Ë“™’‚∏ˆchannel£¨ƒ«√¥’‚∏ˆgc≤Ÿ◊˜æÕ±ª÷–∂œ¡À*/
		...
		if (flag_invoke_gc==1)
		{
			...
			{
				flag_gc=interrupt_gc(ssd,channel,chip,die,plane,gc_node);             /*µ±“ª∏ˆÕÍ’˚µƒgc≤Ÿ◊˜ÕÍ≥… ±£®“—æ≠≤¡≥˝“ª∏ˆøÈ£¨ªÿ ’¡À“ª∂® ˝¡øµƒflashø’º‰£©£¨∑µªÿ1£¨Ω´channel…œœ‡”¶µƒgc≤Ÿ◊˜«Î«ÛΩ⁄µ„…æ≥˝*/
				if (flag_gc==1)
				{
					delete_gc_node(ssd,channel,gc_node);
				}
...
/************************************************************************************************************
*flag”√¿¥±Íº«gc∫Ø ˝ «‘⁄ssd’˚∏ˆ∂º «idleµƒ«Èøˆœ¬±ªµ˜”√µƒ£®1£©£¨ªπ «»∑∂®¡Àchannel£¨chip£¨die£¨plane±ªµ˜”√£®0£©
*Ω¯»Îgc∫Ø ˝£¨–Ë“™≈–∂œ «∑Ò «≤ªø…÷–∂œµƒgc≤Ÿ◊˜£¨»Áπ˚ «£¨–Ë“™Ω´“ª’˚øÈƒø±ÍblockÕÍ»´≤¡≥˝∫Û≤≈À„ÕÍ≥…£ª»Áπ˚ «ø…÷–∂œµƒ£¨
*‘⁄Ω¯––GC≤Ÿ◊˜«∞£¨–Ë“™≈–∂œ∏√channel£¨die «∑Ò”–◊”«Î«Û‘⁄µ»¥˝≤Ÿ◊˜£¨»Áπ˚√ª”–‘Úø™ º“ª≤Ω“ª≤Ωµƒ≤Ÿ◊˜£¨’“µΩƒø±Í
*øÈ∫Û£¨“ª¥Œ÷¥––“ª∏ˆcopyback≤Ÿ◊˜£¨Ã¯≥ˆgc∫Ø ˝£¨¥˝ ±º‰œÚ«∞Õ∆Ω¯∫Û£¨‘Ÿ◊ˆœ¬“ª∏ˆcopybackªÚ’ﬂerase≤Ÿ◊˜
*Ω¯»Îgc∫Ø ˝≤ª“ª∂®–Ë“™Ω¯––gc≤Ÿ◊˜£¨–Ë“™Ω¯––“ª∂®µƒ≈–∂œ£¨µ±¥¶”⁄”≤„–÷µ“‘œ¬ ±£¨±ÿ–ÎΩ¯––gc≤Ÿ◊˜£ªµ±¥¶”⁄»Ì„–÷µ“‘œ¬ ±£¨
*–Ë“™≈–∂œ£¨ø¥’‚∏ˆchannel…œ «∑Ò”–◊”«Î«Û‘⁄µ»¥˝(”––¥◊”«Î«Ûµ»¥˝æÕ≤ª––£¨gcµƒƒø±Ídie¥¶”⁄busy◊¥Ã¨“≤≤ª––)£¨»Áπ˚
*”–æÕ≤ª÷¥––gc£¨Ã¯≥ˆ£¨∑Ò‘Úø…“‘÷¥––“ª≤Ω≤Ÿ◊˜
************************************************************************************************************/
unsigned int gc(struct ssd_info *ssd,unsigned int channel, unsigned int flag)
{
	...
	else                                                                               /*÷ª–Ë’Î∂‘ƒ≥∏ˆÃÿ∂®µƒchannel£¨chip£¨dieΩ¯––gc«Î«Ûµƒ≤Ÿ◊˜(÷ª–Ë∂‘ƒø±ÍdieΩ¯––≈–∂®£¨ø¥ «≤ª «idle£©*/
	{
		if ((ssd->parameter->allocation_scheme==1)||((ssd->parameter->allocation_scheme==0)&&(ssd->parameter->dynamic_allocation==1)))
		{
			if ((ssd->channel_head[channel].subs_r_head!=NULL)||(ssd->channel_head[channel].subs_w_head!=NULL))    /*∂”¡–…œ”–«Î«Û£¨œ»∑˛ŒÒ«Î«Û*/
			{
				return 0;
			}
		}
...
/**********************************************************
*≈–∂œ «∑Ò”–◊”«Î«Û—™“©channel£¨»Ùπ˚√ª”–∑µªÿ1æÕø…“‘∑¢ÀÕgc≤Ÿ◊˜
*»Áπ˚”–∑µªÿ0£¨æÕ≤ªƒ‹÷¥––gc≤Ÿ◊˜£¨gc≤Ÿ◊˜±ª÷–∂œ
***********************************************************/
int decide_gc_invoke(struct ssd_info *ssd, unsigned int channel)      
{
	...
	if ((ssd->channel_head[channel].subs_r_head==NULL)&&(ssd->channel_head[channel].subs_w_head==NULL))    /*’‚¿Ô≤È’“∂¡–¥◊”«Î«Û «∑Ò–Ë“™’º”√’‚∏ˆchannel£¨≤ª”√µƒª∞≤≈ƒ‹÷¥––GC≤Ÿ◊˜*/
	{
		return 1;                                                                        /*±Ì æµ±«∞ ±º‰’‚∏ˆchannel√ª”–◊”«Î«Û–Ë“™’º”√channel*/
	}
	else
	{
		...
		else if (ssd->channel_head[channel].subs_r_head!=NULL)
		{
			...
			while (sub!=NULL)
			{
				if (sub->current_state==SR_WAIT)                                         /*’‚∏ˆ∂¡«Î«Û «¥¶”⁄µ»¥˝◊¥Ã¨£¨»Áπ˚À˚µƒƒø±Ídie¥¶”⁄idle£¨‘Ú≤ªƒ‹÷¥––gc≤Ÿ◊˜£¨∑µªÿ0*/
				{
